...
Juan types an address and presses **Run comps**. Claude searches the web live (Anthropic API with web search and web fetch): it looks up the property, finds sold comps, opens the listing pages to confirm the numbers, adds source links, and flags risks. The page then scores and adjusts the comps and shows:
...
| `api/comps.js` | Calls Claude (`claude-opus-5` with live web search) and returns the comps as JSON. Your API key stays on the server. |
...
1. Get an Anthropic API key at console.anthropic.com → API Keys. Add a payment method. A run with web search typically costs well under a dollar.
...
3. Under **Settings → Environment Variables**, add:
   ...
   - optional: `CLAUDE_MODEL` (default `claude-opus-5`), `CLAUDE_EFFORT` (`low` / `medium` / `high`, default `high`), `CLAUDE_MAX_SEARCHES` (default 12)
4. Deploy. Send Juan the URL. The first run asks for the team password once.

A run usually takes 1–4 minutes, because Claude is searching and reading listing pages. `vercel.json` allows up to 5 minutes.
...
2. Add a second pass: a Claude "Chief Appraiser" call that re-checks the comps the first pass found.
